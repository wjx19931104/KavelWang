...
~function () {
    ...
    //截取前n个字节的字符返回，isD若是true在截取之后，添加省略号"..."后返回，否则截取后直接返回截取的字符串
    strPro.mySub = function (index,isD) {
    ...
    //将2015-08-09 11:35:45等这类的日期格式化为 X年X月X日 时：分：秒的形式
    ...
    //获取一个字符串URL中的参数，以对象{key:value}的形式返回URL的参数
    ...
    //判断一个属性是否是这个对象的公有方法，用法和hasOwnProperty一样
